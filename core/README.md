...
- 读取：通过 `libarchive` 实现对 `.7z` 的读取（含密码读取，密码错误时可识别）。
- 写入：写入阶段会先将文件镜像到临时目录，在 `close()` 时调用系统中的 `7z/7za` 命令行工具打包为真实的 `.7z` 文件。
  - 压缩：支持 `LZMA`/`LZMA2`/`Deflate`，将分别映射为 `-m0=lzma/lzma2/deflate`。
  - 加密：当设置密码且加密算法为任意 `AES*` 时，使用 `7z` 的 AES-256，并启用头加密（`-p<password> -mhe=on`）。

注意：生成真实 `.7z` 需要系统环境 `PATH` 中可用的 `7z`（或 `7za`）。若未安装，库仍可使用内存模式完成读写（供测试/临时使用），但不会产生实际 `.7z` 文件。
