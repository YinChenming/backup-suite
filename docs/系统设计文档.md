...
- ZipCrypto 加密流程：
    ...
    1. `Streaming`: 对正文每字节，先用 key 生成 keystream 字节，再 XOR 数据，同时更新三个 key（key0、key1、key2）。
    1. 解密同流程，先解出头部并校验第12字节；失败则判定密码错误。
- RC4 加密流程：
    1. `KeySetup`: 以密码或派生 key 执行 KSA 初始化 256-byte S 盒；`i=j=0`。
    1. `HeaderGen`: 生成16字节随机头存入 zip 标准的加密头中，并记录原始密码的 CRC32 字节，解密时通过计算解密后的 CRC32 校验密码正确性。
...
- 原则：
    - 兼容性：字段与 tar/zip 标准字段对齐，可映射到 C++ 结构（`FileEntityMeta`、`CentralDirectoryEntry` 等）。
